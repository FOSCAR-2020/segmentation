...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str='', save_path = None):
    """
    ...
    with timer.env('Copy'):
        ...
        mapping={0:1,1:2,2:4,3:3,4:6,5:5,6:6,7:6,8:6}
        ...
        save_path = save_path[:-4] + '.json'
        # print("save_path : ", img_name)
        annotations = list()

        for idx, (_class, _score, _boxes, _mask) in enumerate(zip(classes, scores, boxes, masks)):
            if _class < 9 and idx < j:
                category_name = cfg.dataset.class_names[_class]
                category_id = mapping[_class]

                ground_truth_binary_mask = np.array(_mask.cpu(), dtype=np.uint8)
                #print("gt_bin.shape : ", ground_truth_binary_mask.shape)
        ...
        with open(save_path, 'w') as f:
            json.dump(annotations, f, indent=2)
                ...
                #cv2.imshow("sex", ground_truth_binary_mask*255)
                #cv2.waitKey(-1)

                #print(category_name)
                #print(category_id)

        # image id, category_id, segmentation

        # for new_class in classes:
        #     if new_class < 9:
        #         mapping={0:1,1:2,2:4,3:3,4:6,5:5,6:6,7:6,8:6}
        #         _class = cfg.dataset.class_names[new_class]
        #
        #         print(_class, ' ' , mapping[new_class])
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    img_numpy = prep_display(preds, frame, None, None, undo_transform=False, save_path=save_path_1)
